...
# Copy image from unrecognized bucket to faceappntust/registered/
try:
...
# Optionally delete original unrenamed image from registered (if already present)
try:
    if src_bucket == MAIN_BUCKET and src_key != dst_key:
        s3.delete_object(Bucket=MAIN_BUCKET, Key=src_key)
        print(f"🗑️ Old image s3://{src_bucket}/{src_key} deleted.")
except Exception as e:
    print(f"⚠️ Failed to delete old image: {e}")

# Index face using renamed image
try:
